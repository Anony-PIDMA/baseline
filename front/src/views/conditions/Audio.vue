<!-- src/components/conditions/Audio.vue -->
<template>
  <div class="audio-condition-container">
    <h1 class="main-title">Audio</h1>

    <!-- Keyword input box (always visible) -->
    <div class="section">
      <h2 class="section-title">Content Detected in Audio</h2>
      <input 
        type="text" 
        v-model="keyword" 
        id="keyword-input" 
        placeholder="e.g.: Doorbell sound" 
        class="keyword-input"
        @focus="onActionFocus"
        @input="onActionInput"
      />
...
const generateText = () => {
  ...
  if (keyword.value) {
    description += `The audio detected the content "${keyword.value}"`
  }
