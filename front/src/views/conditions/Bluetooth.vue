<template>
  <div class="bluetooth-condition-container">
    <h1 class="main-title">Bluetooth</h1>
    ...
    <!-- Bluetooth device name selection dropdown -->
    <div class="section">
      <h2 class="section-title">Select Bluetooth Device</h2>
      <select v-model="bluetoothName" class="bluetooth-select" @change="onBluetoothSelect">
        <option value="" disabled>Select a Bluetooth device</option>
        <option v-for="device in bluetoothDevices" :key="device.name" :value="device.name">{{ device.name }}</option>
      </select>
    </div>
...
onMounted(() => {
  ...
  if (id) {
    ...
    if (condition) {
      ...
      bluetoothName.value = condition.details.bluetoothName || ''
    }
  }

  // Load the device info (Bluetooth devices)
  const deviceInfo = getDeviceInfo()
  bluetoothDevices.value = deviceInfo.bluetoothDevices
})
...
const onBluetoothSelect = () => {
  // Add log record: Bluetooth device selected
  addLog('Bluetooth Device Selected', { bluetoothName: bluetoothName.value })
}

// Navigate back to ConditionSelection.vue
const navigateBack = () => {
  const id = route.query.id
  addLog('Go Back', { from: 'Bluetooth', to: 'ConditionSelection' })  // Log the back action
...
const confirmSelection = () => {
  ...
  if (!isNaN(id)) {
    const details = { 
      ...
      bluetoothName: bluetoothName.value,
      text: generateText()
    }
...
const generateText = () => {
  ...
  if (selectedOption.value === 'Connected') {
    description += `Connect to the "${bluetoothName.value}" Bluetooth device`
  } else if (selectedOption.value === 'Disconnected') {
    description += `Disconnect from the "${bluetoothName.value}" Bluetooth device`
  }
